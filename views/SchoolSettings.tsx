...
import { Settings, Save, GraduationCap, X, ChevronDown, Building2, Link, Copy, ExternalLink, ToggleLeft, ToggleRight } from 'lucide-react';
...
const SchoolSettings: React.FC<SchoolSettingsProps> = ({ school, config, onUpdateSettings, onRefresh }) => {
  ...
  const handleTestLink = () => {
    // Tenta abrir com hash como fallback para ambientes de preview
    const testUrl = `${window.location.origin}/#/matricular/${localSettings.slug}`;
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-10 animate-in fade-in duration-500">
      ...
      <div className="flex gap-6 border-b border-slate-100 overflow-x-auto scrollbar-hide relative no-radius-important">
        {tabs.map((tab) => (
          ...
          >
            {tab.label}
            {activeTab === tab.id && <div className="absolute bottom-0 left-0 w-full h-[2.5px] bg-slate-900 z-10" />}
          </button>
        ))}
      </div>
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-3 bg-brand-purple/10 text-brand-purple rounded-xl"><Link size={24} /></div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-[11px] italic pr-2 border-r border-slate-200">/matricular/</div>
                    <input 
                      type="text"
                  ...
                  <p className="text-[9px] text-slate-400 italic ml-1">Ex: arena-soccer-sp</p>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleCopyLink} className="p-3 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-indigo-600 transition-all shadow-sm"><Copy size={18} /></button>
                    <button onClick={handleTestLink} className="p-3 bg-slate-900 text-white rounded-xl hover:bg-black transition-all shadow-lg"><ExternalLink size={18} /></button>
                  </div>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  placeholder="Ex: Solicitação enviada com sucesso! Entraremos em contato."
        ...
        <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none font-bold text-slate-800 text-sm italic uppercase" value={localSettings.name} onChange={e => setLocalSettings({...localSettings, name: e.target.value})} />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nome do Gestor</label>
              <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none font-bold text-slate-800 text-sm italic" value={localSettings.managerName} onChange={e => setLocalSettings({...localSettings, managerName: e.target.value})} />
            </div>
          </div>
        </div>
      )}
